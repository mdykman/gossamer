...
var filepath = '/rq/scon/filelist';
	...
	function createXMLHTTPObject() {
		var xmlobj;
		if(XMLHttpRequest) xmlobj = new XMLHttpRequest();
		if((!xmlobj)) {
			if(ActiveXObject) {
	...
	function checkSubmission(frm) {
		if (newCalled) {
			...
			var wn = window.open('',
					'codewindow','toolbar=n,status=n,scrollbars',true);
			var xhr= $.post(sconpath, args, function() {
					if(xhr.responseXML) {
						Didi.Ajax.updateXML(wn.document.body,xhr.responseXML);
					} else {
						$(wn.document.body).text(xhr.responseText);
					}
					wn.focus();
					//wn.alert('status: ' + xhr.status);
					});
			return false;
		}
		else if (listCalled) {
